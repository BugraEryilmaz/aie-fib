...
void inline writeRegister64(xrt::ip &ip, uint64_t offset, uint64_t value) {
  ...
  ip.write_register(offset + 4,
                    static_cast<uint32_t>((value >> 32) & 0xFFFFFFFF));
}

xrt::bo *special_bo;
// fib_task ** special_map;

template <typename T>
void inline initializeScheduler(
    std::vector<T> base_task_data, uint64_t bufferSize, uint64_t rAddr_shift,
    uint64_t maxLength_shift, uint64_t fifoTailReg_shift,
    uint64_t fifoHeadReg_shift, uint64_t currLen_shift, xrt::ip &ip1,
    xrt::device &device, int memoryBank, xrt::bo *preallocated_bo = nullptr,
    // pre allocated map
  ...
  // fill with zeros but use explicit pointer conversion to avoid issues with
  // std::fill and non-trivial types
  std::fill(reinterpret_cast<uint8_t *>(scheduler_virtual_queue_map),
            reinterpret_cast<uint8_t *>(scheduler_virtual_queue_map) +
                vector_size_bytes,
            0);

  // if base_task_data is not empty, copy the base_task_data to the head of the
  // scheduler_virtual_queue_map
  if (!base_task_data.empty()) {
    auto special_map_lval = special_bo->map<T *>();

    auto special_map = &(special_map_lval);

    std::copy(base_task_data.begin(), base_task_data.end(),
              *special_map + 1024 / sizeof(T) - 1);
    // std::copy(base_task_data.begin(), base_task_data.end(),
    //           scheduler_virtual_queue_map);
    special_bo->sync(XCL_BO_SYNC_BO_TO_DEVICE);
  }
    ...
    for (size_t i = 0; i < sizeof(T); i++) {
      std::cout << std::hex
                << static_cast<uint32_t>(
                       reinterpret_cast<uint8_t *>(&verify_map[0])[i])
                << " ";
    }
  ...
  std::cout << "Scheduler virtual queue address: " << std::hex << addr
            << std::dec << std::endl;
  if (!base_task_data.empty()) {
    addr = special_bo->address() + 1024 - sizeof(T);
  }
  ...
  if (val != addr) {
    std::cerr << "Error: rAddr register value mismatch. Expected: " << std::hex
              << addr << ", Got: " << val << std::dec << std::endl;
  }
  val = readRegister64(ip1, maxLength_shift);
  if (val != bufferSize) {
    std::cerr << "Error: maxLength register value mismatch. Expected: "
              << std::hex << bufferSize << ", Got: " << val << std::dec
              << std::endl;
  }
  val = readRegister64(ip1, fifoTailReg_shift);
  if (val != base_task_data.size()) {
    std::cerr << "Error: fifoTailReg register value mismatch. Expected: "
              << std::hex << base_task_data.size() << ", Got: " << val
              << std::dec << std::endl;
  }
  val = readRegister64(ip1, fifoHeadReg_shift);
  if (val != 0x0) {
    std::cerr << "Error: fifoHeadReg register value mismatch. Expected: "
              << std::hex << 0x0 << ", Got: " << val << std::dec << std::endl;
  }
  val = readRegister64(ip1, currLen_shift);
  if (val != base_task_data.size()) {
    std::cerr << "Error: currLen register value mismatch. Expected: "
              << std::hex << base_task_data.size() << ", Got: " << val
              << std::dec << std::endl;
  }
}
...
void inline logRpause(xrt::ip &ip, uint64_t offset, const std::string &name) {
  ...
  std::cout << "rPause " << name << ": " << std::hex << value << std::dec
            << std::endl;
}
...
template <typename T>
void inline initializeAllocator(uint64_t bufferSize, uint64_t rAddr_shift,
                                uint64_t availableSize_shift, xrt::ip &ip1,
                                xrt::device &device, int memoryBank_0,
                                int memoryBank_1,
  ...
  // fill with zeros but use explicit pointer conversion to avoid issues with
  // std::fill and non-trivial types
  std::fill(reinterpret_cast<uint8_t *>(allocator_virtual_queue_map),
            reinterpret_cast<uint8_t *>(allocator_virtual_queue_map) +
                vector_size_bytes,
            0);
  ...
  std::cout << "Allocator virtual queue address: " << std::hex << addr
            << std::dec << std::endl;
  ...
  std::cout << "Allocator address queue address: " << std::hex << addr_queue
            << std::dec << std::endl;
  ...
  if (val != addr_queue) {
    std::cerr << "Error: rAddr register value mismatch. Expected: " << std::hex
              << addr_queue << ", Got: " << val << std::dec << std::endl;
  }
  val = readRegister64(ip1, availableSize_shift);
  if (val != addresses.size()) {
    std::cerr << "Error: availableSize register value mismatch. Expected: "
              << std::hex << addresses.size() << ", Got: " << val << std::dec
              << std::endl;
  }
}
...
int main(int argc, char **argv) {
  ...
  // std::cout << "Resetting aie graph " << std::endl;
  // // Initialize AIE graph explicitly
  // auto aie_device = xrt::aie::device(device_index);
  // std::cout << "Resetting aie graph " << std::endl;
  // auto aie_uuid = aie_device.load_xclbin(binaryFile);
  // std::cout << "Resetting aie graph " << std::endl;
  // auto graph = xrt::graph(aie_device, aie_uuid, "simpleGraph");
  // std::cout << "Resetting aie graph " << std::endl;
  // graph.reset();
  // std::cout << "Resetting aie graph " << std::endl;
  // graph.run();

  std::cout << "Create the IP object for fibonacci:{fibonacci_1}" << std::endl;

  auto ip1 = xrt::ip(device, uuid, "fibonacci:{fibonacci_1}");

  std::cout << "Log the rPause registers before starting the servers"
            << std::endl;

  // Log the rPause registers before starting the servers
  logAllRpause(ip1);

  // A loop to read all data at the register file from address 0x0 to 0x100
  // logging the address and the data as hex 64-bit values std::cout << "Reading
  // all registers from 0x0 to 0x100" << std::endl; for(uint64_t addr = 0x0;
  // addr <= 0x100; addr += 0x4){
  //     uint64_t data = ip1.read_register(addr);
  //     std::cout << "Address: " << std::hex << addr << " Data: " << data <<
  //     std::dec << std::endl;
  // }

  sum_task sum_task_0 = {
      4, 0, 0, 0, {0}}; // Create a continuation task of sum_task
  auto sum_task_bo =
      xrt::bo(device, sizeof(sum_task_0),
              0); // Move it to a buffer object to move to the FPGA
  ...
  uint64_t sum_task_addr =
      sum_task_bo.address(); // Get the physical address of the sum_task_bo
  std::cout << "sum_task address: " << std::hex << sum_task_addr << std::dec
            << std::endl;
  ...
  if (std::memcmp(&sum_task_0, &sum_task_readback, sizeof(sum_task_0)) != 0) {
    std::cerr << "Error: sum_task structure mismatch after write and readback"
              << std::endl;
    return -1;
  }

  fib_task args = {sum_task_addr + 0x4,
                   fib_value,
                   {0}}; // Create the fib base task using fib args
  ...
  auto special_bo_lval = xrt::bo(device, 1024, 0);
  special_bo = &special_bo_lval;
  ...
  initializeScheduler<fib_task>(
      base_task_data, buf_size, REG_FIB_SCHEDULER_0_RADDR,
      REG_FIB_SCHEDULER_0_MAXLEN, REG_FIB_SCHEDULER_0_FIFOTAILREG,
      REG_FIB_SCHEDULER_0_FIFOHEADREG, REG_FIB_SCHEDULER_0_CURRLEN, ip1, device,
      2, &fib_task_bo, &fib_task_map);
  ...
  initializeScheduler<sum_task>(
      {}, buf_size, REG_SUM_SCHEDULER_0_RADDR, REG_SUM_SCHEDULER_0_MAXLEN,
      REG_SUM_SCHEDULER_0_FIFOTAILREG, REG_SUM_SCHEDULER_0_FIFOHEADREG,
      REG_SUM_SCHEDULER_0_CURRLEN, ip1, device, 1, &sum_scheduler_0_bo,
      &sum_scheduler_0_map);
  ...
  initializeScheduler<sum_task>(
      {}, buf_size, REG_SUM_SCHEDULER_1_RADDR, REG_SUM_SCHEDULER_1_MAXLEN,
      REG_SUM_SCHEDULER_1_FIFOTAILREG, REG_SUM_SCHEDULER_1_FIFOHEADREG,
      REG_SUM_SCHEDULER_1_CURRLEN, ip1, device, 6, &sum_scheduler_1_bo,
      &sum_scheduler_1_map);
  ...
  auto allocator_address_queue =
      xrt::bo(device, buf_size * sizeof(uint64_t), 0);
  auto allocator_address_queue_map = allocator_address_queue.map<uint64_t *>();

  initializeAllocator<sum_task>(
      buf_size, REG_SUM_ALLOCATOR_0_RADDR, REG_SUM_ALLOCATOR_AVAILABLESIZE, ip1,
      device, 3, 4, &allocator_virtual_queue, &allocator_virtual_queue_map,
      &allocator_address_queue, &allocator_address_queue_map);
  ...
  // In a loop readback the sum_task structure from the FPGA and log the
  // _counter, f1, f2 values until f1 is not zero and print f1

  sum_task result = {0, 0, 0, 0, 0, 0};

  sum_task_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  std::copy(sum_task_map, sum_task_map + 1, &result);
  std::cout << "sum_task _counter: " << result._counter << " f1: " << result.f1
            << " f2: " << result.f2 << std::endl;
  ...
  do {
    sum_task_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
    std::copy(sum_task_map, sum_task_map + 1, &result);
    std::cout << "sum_task _counter: " << result._counter
              << " f1: " << result.f1 << " f2: " << result.f2 << std::endl;
  ...
  std::cout << "Final Result: Fibonacci(" << fib_value << ") = " << result.f1
            << std::endl;
  ...
  //     bo0_map[i] = i;
  //     bo1_map[i] = i;
  //     bufReference[i] = bo0_map[i] + bo1_map[i]; //Generate check data for
  //     validation
  // }
  ...
  //     i = i + 1;
  //     std::cout << "Read Loop iteration: " << i << " Kernel Done: " <<
  //     krnl_done << " Kernel Idle: " << krnl_idle << "\n";
  // }
